...
int main()
{
    ...
    std::shared_ptr<TDisplayInterface> display = TGameLocalDisplay::getInstance();
...
Uint32 mya_callbackfunc(Uint32 interval, void *param) {
    SDL_Event event;
...
int main() {
    ...
    SDL_TimerID my_timer_id = SDL_AddTimer(1000, mya_callbackfunc, nullptr);
    ...
    while (quit) {
        ...
        for (int y = 0 ; y < board.size() ; y++) {
            ...
            for (int x = 0 ; x < row.size() ; x++) {
                ...
                if (board[y][x].getType() == UnitType::Fill) {
                    SDL_Rect rect;
                    rect.y = 50 + y * 30;
                    rect.x = 50 + x * 30;
                    rect.h = 30;
