...
TClientService::TClientService(ACE_Reactor *reactor)
    : ACE_Event_Handler(reactor), peer_(ACE_INVALID_HANDLE), state_(C_INIT) {
}

TClientService::~TClientService(void) {
}

void
TClientService::state(CSTATE state) {
    this->state_ = state;
    if (C_SUCCESS != this->state_) return;
    this->reactor()->register_handler(this, ACE_Event_Handler::READ_MASK);
}

TClientService::CSTATE
TClientService::state() const {
    return this->state_;
}
...
ACE_HANDLE
TClientService::get_handle(void) const {
    return this->peer_.get_handle();
}

int
TClientService::handle_input(ACE_HANDLE fd/* = ACE_INVALID_HANDLE*/) {
    const int BUF = 1024;
...
int
TClientService::handle_close(ACE_HANDLE handle, ACE_Reactor_Mask close_mask) {
...
int
TClientService::send(void *data, ssize_t len) {
...
int
TClientService::handle_output(ACE_HANDLE fd/* = ACE_INVALID_HANDLE*/) {
