...
/* Windows targets do not have lrint_, so provide a local inline version */
#if defined(_MSC_VER)
...
#pragma warning(pop)
#else
#error lrint_ needed for MSVC on non X86/AMD64/ARM targets.
#endif
#endif
...
int aaellipseRGBA(SDL_Renderer * renderer, Sint16 x, Sint16 y, Sint16 rx, Sint16 ry, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	*/
	...
	od = (Sint16)lrint_(sab*0.01) + 1; /* introduce some overdraw */
	dxt = (Sint16)lrint_((double)a2 / sab) + od;
	...
	dyt = (Sint16)lrint_((double)b2 / sab ) + od;    
...
int bezierRGBA(SDL_Renderer * renderer, const Sint16 * vx, const Sint16 * vy, int n, int s, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	*/
	t=0.0;
	x1=(Sint16)lrint_(_evaluateBezier(x,n+1,t));
	y1=(Sint16)lrint_(_evaluateBezier(y,n+1,t));
	for (i = 0; i <= (n*s); i++) {
		t += stepsize;
